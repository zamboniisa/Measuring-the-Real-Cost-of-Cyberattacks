...
path = '/Users/isabellazamboni/Desktop/tesi_aggiornato/tesi_codice/data'
...
import os, calendar
from datetime import datetime, timedelta
from time import sleep  # opzionale: puoi rimuoverlo/scommentarlo dove indicato
import re
import os
os.environ["GCP_PROJECT"] = "tesi-isa-1"
...
# Vertex AI (per la tua funzione generate)
import vertexai
...
def _get_bq_client() -> bigquery.Client:
    """
    Inizializza il client BigQuery usando:
      - GOOGLE_APPLICATION_CREDENTIALS (se presente) o ADC locale
      - Progetto da GCP_PROJECT o GOOGLE_CLOUD_PROJECT
    """
    project = os.environ.get("GCP_PROJECT") or os.environ.get("GOOGLE_CLOUD_PROJECT")
    if not project:
        raise RuntimeError(
            "Devi impostare la variabile d'ambiente GCP_PROJECT (o GOOGLE_CLOUD_PROJECT) "
            "con l'ID del tuo progetto GCP."
        )
    ...
    # ADC (es. gcloud auth application-default login)
    return bigquery.Client(project=project)


def _rx(s: str) -> str:
    """Escape sicuro per regex (minuscolo)."""
    return re.escape(s.strip().lower())


def _build_keyword_predicate(keywords):
    """
    Predicato SQL per cercare temi/keyword cyber in GKG.
    - V2Themes è pipe-delimited e spesso maiuscolo → cerchiamo |THEME|
    - Manteniamo anche un fallback substring case-insensitive
    """
    parts = []
    for k in keywords:
        k = k.strip()
        if not k:
            continue
        # Variante per themes (UPPER + delimitatore |)
        k_upper = re.escape(k.upper())
        parts.append(
            f"REGEXP_CONTAINS(CONCAT('|', UPPER(V2Themes), '|'), r'\\|{k_upper}\\|')"
        )
        # Fallback substring su V2Themes lowercase (spazi e underscore)
        k_low = _rx(k)
        k_us  = _rx(k.replace(" ", "_"))
        parts.append(f"REGEXP_CONTAINS(LOWER(V2Themes), r'{k_low}')")
        if k_us != k_low:
            parts.append(f"REGEXP_CONTAINS(LOWER(V2Themes), r'{k_us}')")

    return "(" + " OR ".join(parts) + ")" if parts else "TRUE"


def _build_name_predicate(company_name: str):
    """
    Predicato per intercettare il nome azienda in vari campi GKG:
    V2Persons, V2Organizations, V2Locations, AllNames (tutti pipe-delimited).
    Match case-insensitive su substring (regex escaped).
    """
    name = _rx(company_name)
    fields = ["V2Persons", "V2Organizations", "V2Locations", "AllNames"]
    ors = [f"REGEXP_CONTAINS(LOWER({f}), r'{name}')" for f in fields]
    return "(" + " OR ".join(ors) + ")"


def _query_gkg_window(
    client: bigquery.Client,
    company_name: str,
    start_dt: datetime,
    end_dt: datetime,
    keywords,
    limit_per_window: int = 100000,
) -> pd.DataFrame:
    """
    Query su gdelt-bq.gdeltv2.gkg per l'intervallo [start_dt, end_dt),
    filtrando per nome azienda e keyword cyber.
    """
    # GKG.DATE è INT64 formato YYYYMMDDHHMMSS
    start_str = start_dt.strftime("%Y%m%d%H%M%S")
    end_str   = end_dt.strftime("%Y%m%d%H%M%S")

    kw_pred   = _build_keyword_predicate(keywords)
    name_pred = _build_name_predicate(company_name)

    sql = f"""
    SELECT
      DATE, SourceCommonName, DocumentIdentifier,
      V2Themes, V2Persons, V2Organizations, V2Locations, V2Tone, AllNames
    ...
    WITH base AS (
        SELECT
            DATE, SourceCommonName, DocumentIdentifier,
            V2Themes, V2Persons, V2Organizations, V2Locations, V2Tone, AllNames,
            -- strict token hits in themes
            ARRAY_LENGTH(REGEXP_EXTRACT_ALL(CONCAT('|', UPPER(V2Themes), '|'), @themes_token_regex)) AS kw_hits_token,
            -- loose substring hits in themes
            ARRAY_LENGTH(REGEXP_EXTRACT_ALL(LOWER(V2Themes), @themes_substring_regex)) AS kw_hits_sub
        FROM `gdelt-bq.gdeltv2.gkg`
        WHERE DATE >= @start AND DATE < @end
        )
    SELECT * FROM base
    WHERE (kw_hits_token >= @min_kw OR kw_hits_sub >= @min_kw_loose)
    ORDER BY DATE DESC
    LIMIT @lim
    """
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            bigquery.ScalarQueryParameter("start", "INT64", int(start_str)),
            bigquery.ScalarQueryParameter("end",   "INT64", int(end_str)),
            bigquery.ScalarQueryParameter("lim",   "INT64", int(limit_per_window)),
        ]
    )
...
# =========================
# Funzione principale (stessa firma)
# =========================
def download_csv(name, date, days_param, try_ = False):
    """
    Esegue query su GDELT GKG (BigQuery) per un dato nome azienda,
    scorrendo finestre temporali come il tuo codice originale e salvando un CSV.
    """
    # --- parse "Aug 2022" into numeric month/year ---
    try:
        mon_str, yr_str = date.split()
    ...
    # --- compute previous/next month/year ---
    if month == 1:
        prev_month, prev_year = 12, year - 1
    else:
        prev_month, prev_year = month - 1, year
    ...
    # --- modalità di test (try_) su una singola finestra ---
    if try_:
        start_time = datetime(prev_year, month, 15, 0, 0)
        print(f'Trying for {days_param} days for company {name}')
        end_time = start_time + timedelta(days=days_param)
        df_try = _query_gkg_window(
            client, name, start_time, end_time, keywords, limit_per_window=200_000
        )
        tot = len(df_try)
        print(f'Number of articles (GKG rows) found in this date range: {tot}')
        return tot

    # --- scansione a finestre come nel tuo codice ---
    print(f"Fetching for company {name} from {period_start:%Y-%m-%d} to {period_end:%Y-%m-%d}")
    combined_all = pd.DataFrame()
    start_time = period_start

    while start_time < period_end:
        # sleep(1)  # NON necessario con BigQuery; tienilo solo se vuoi "throttling"
        end_time = start_time + timedelta(days=days_param)
        if end_time > period_end:
            end_time = period_end
        ...
        try:
            df_win = _query_gkg_window(
                client, name, start_time, end_time, keywords, limit_per_window=200_000
            )
            print(f'Number of articles (GKG rows) in this window: {len(df_win)}')
            if not df_win.empty:
                combined_all = pd.concat([combined_all, df_win], ignore_index=True)
        except Exception as e:
            print(f"Errore durante la query BigQuery: {e}")
    ...
    try:
        ...
        out_path = f"data/{name}/gdelt_{date.replace(' ', '_')}.csv"
        combined_all.to_csv(out_path, index=False)
        print(f"Saved {len(combined_all)} total rows to {out_path}")
        print(f'Number of articles found for {name}: {len(combined_all)}')
    except Exception as e:
        print(f'Number of articles found for {name}: 0')
        print(f'Error: {e}')
...
def download_all(company, csv_name, n = 0, start = 0, skip = 0, devided = False):
    uploads_path = '/Users/isabellazamboni/Desktop/tesi_aggiornato/tesi_codice/data'
    ...
    for link in tqdm(df["DocumentIdentifier"].astype(str)):
